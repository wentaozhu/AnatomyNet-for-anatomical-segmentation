...
class UpSEBasicBlock3D(nn.Module):
    def __init__(self, inplanes1, inplanes2, planes, stride=1, downsample=None, reduction=16):
        ...
        self.stride = stride
        self.conv1x1x1 = nn.Conv3d(inplanes2, inplanes3, kernel_size=1, stride=1)#, padding=1)
...
#     print(y_pred.sum(), y_true.sum())
    ...
    for dice in avgdice: 
        if dice != -1:
            assert 0 <= dice <= 1
    return avgdice
model = ResNetUNET3D(SEBasicBlock3D, UpSEBasicBlock3D, UpBasicBlock3D, 3, num_classes=9+1, in_channel=1).cuda() 
lossweight = np.array([2.22, 1.31, 1.99, 1.13, 1.93, 1.93, 1.0, 1.0, 1.90, 1.98], np.float32)
savename = './model/unet10pool3e2e_seres18_sum_pet_wmask_rmsp_'
